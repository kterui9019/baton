...
test("recoverOnStartup: result=needs_info は needs_info 確定・質問コメント投稿・アクティビティ更新", async () => {
  ...
  for (const c of patches) {
    const body = c.args[c.args.indexOf("-d") + 1] ?? "";
    expect(body).not.toContain('"Status"');
  }
  expect(
    patches.some((c) => (c.args[c.args.indexOf("-d") + 1] ?? "").includes("要回答")),
  ).toBe(true);
