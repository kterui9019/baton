...
// ---- 2. prReconcile: CI 全 green ----
// prReconcile は use-cases/orchestrator.ts の内部関数のため、公開 API の tick() 経由で検証する
// （tick は reconcile → terminalCleanup → prReconcile → queryCandidates → dispatchLoop の順に走るが、
//  候補は常に空を返すモックなので prReconcile 以降の副作用は生じない）。

test("prReconcile: CI 全 green → doneLane PATCH + phase=review", async () => {
...
test("prReconcile: CI green だが人間がレーンを動かしていたらレーンは上書きしない（phase は遷移）", async () => {
...
// ---- 3. prReconcile: merged ----

test("prReconcile: merged → mergedLane PATCH + prWatch 削除 + コメント", async () => {
...
test("prReconcile: closed(unmerged) → 通知のみ・レーン維持・prWatch 削除", async () => {
...
// ---- 4. prReconcile: CI failed → マーカー persist + dispatchAutoRework ----

test("prReconcile: CI failed → reworkedSha/count を先に persist し dispatchAutoRework が claim まで到達", async () => {
...
test("prReconcile: 同一 SHA の CI 失敗（reworkedSha 一致）は再発火しない", async () => {
...
test("prReconcile: count >= limit で awaitingHuman=true + 🆘 通知、2 周目は再通知しない", async () => {
...
test("prReconcile: changes requested → マーカー persist + triggerLanes[0] へ戻して自動 rework", async () => {
...
test("prReconcile: maxConcurrent 満杯なら マーカーを進めず持ち越す", async () => {
