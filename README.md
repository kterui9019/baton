<img width="512" height="278" alt="baton" src="https://github.com/user-attachments/assets/43e73772-b461-49c0-90dd-9645fabed9c0" />
...
"kanban": {
  ...
  "github": {
    "owner": "your-org",
    "repos": ["your-repo"],
    "triggerLabels": ["In Progress"],
    "doneLabel": "Human Review",
    "terminalLabels": ["Released", "Canceled"],
    "conditionLabel": ""
  }
}
```

- **ラベルの用意**: `kanban.github.repos` の各リポジトリに、`triggerLabels` / `doneLabel` / `terminalLabels` で指定したラベルを事前に作成してください。既定なら `In Progress`、`Human Review`、`Released`、`Canceled` の 4 つ。プレフィックスは付けません（任意のラベル名を直接指定できます）。
...
- **conditionLabel**（任意）: Notion の `Condition` プロパティに相当する追加フィルタ用ラベル。`""`（既定）なら無効で、`triggerLabels` に対応するラベルさえ付いていれば対象になります。誤発火を防ぎたい場合は例えば `"baton"` のような専用ラベルを作って設定してください（そのラベルが付いた issue のみ対象になります）。
- **pageId 形式**: 内部的に `owner/repo#issue番号`（例: `acme/baton#42`）を使います。`baton status` 等で表示される ID もこの形式です。
- **書き込み内容**: レーン移動は「既存の管理対象ラベル（trigger/done/terminal）を外して新しいラベルを付ける」形で行われます。PR リンクと実行状況は issue への**コメント追記**として記録されます（Notion の `prProperty` に相当する専用フィールドはありません）。
...
- **GitHub Issues**: 対象リポジトリの issue に `triggerLabels` で指定したラベル（既定は `In Progress`。`conditionLabel` 設定時はそちらも）を付ける
- 動かすと数十秒以内に着手されます
...
| `kanban.github.triggerLabels` | string[] | `["In Progress"]` | 実行トリガーとなる issue ラベル（Notion の `triggerLanes` 相当） |
| `kanban.github.doneLabel` | string | `"Human Review"` | CI グリーン後の移動先ラベル（Notion の `doneLane` 相当） |
| `kanban.github.terminalLabels` | string[] | `["Released", "Canceled"]` | このラベルに入ったら worktree と state を掃除（Notion の `terminalLanes` 相当） |
| `kanban.github.conditionLabel` | string | `""` | 追加フィルタ用ラベル（Notion の `conditionProperty`/`conditionValue` 相当）。`""` なら無効（`triggerLabels` のラベルのみで判定） |
