...
- **ラベルの用意**: `kanban.github.repos` の各リポジトリに、`status:` + レーン名（`kanban.triggerLanes` / `doneLane` / `terminalLanes` の各値）のラベルを事前に作成してください。既定なら `status:In Progress`、`status:Human Review`、`status:Released`、`status:Canceled` の 4 つ。
