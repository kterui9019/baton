...
1. **awaitingHuman**: 何もしない（防御。呼び出し側でも除外済み）。
...
- **再開**: attempt=1 の resume（`kind:"needs_info_answer"`）として dispatch。プロンプト先頭に
  ...
  `ci_failure` / `review_changes` と同様、前回 `session_id` が記録されていればネイティブ resume +
...
```jsonc
{
  ...
  "resumePromptTemplate": "prompts/resume.md",  // ci_failure/review_changes/needs_info_answer でネイティブ resume する際の軽量プロンプト（チケット全文を含まない）
