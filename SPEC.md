...
- プロパティ（名前はすべて config で変更可能。`""` を設定すると読み書きをスキップ）:
  - `Title` (title)
  - `Status` (status): 例 TODO / In Progress / Human Review / Released / Canceled
  ...
  "pages": {
    "<page_id>": {
      ...
      "prWatch": {                  // done + PR あり: PR 監視状態
        ...
        "autoReworkCount": 0,       // CI 起因 rework 累計。human rework で 0 リセット
...
1. **Claim**: state に running 記録。ページコメントに `🤖 Claude Code 実行開始 (attempt N) — HH:MM` を投稿。
2. **リポジトリ解決**:
...
4. **プロンプト構築**:
   ...
   - **resume 種別によるテンプレート切り替え**: `resume.kind === "human_rework"` は常に `config.promptTemplate`（チケット全文）で新規セッションを開始する。それ以外（`ci_failure` / `needs_info_answer`）は、前回 dispatch で払い出された `session_id`（state に記録済み）があれば `config.resumePromptTemplate`（デフォルト `prompts/resume.md`。`{{rework}}` の差分情報のみでチケット全文を含まない）を使い、CLI 起動時にネイティブ resume する。`session_id` が未記録ならこの2種でも `promptTemplate` にフォールバックする。
   - `config.systemPromptTemplate`（デフォルト `""` = 無効）が設定されていれば、同じ変数セットで同様に描画し、追加のシステムプロンプトとして用意する（`{{body}}` や `{{rework}}` を含む同じ変数を使えるが、通常はチケット非依存の運用ルール — 呼び出し元の説明・利用可能な補助コマンド等 — を書く用途を想定）。resume 種別にかかわらず常に同じ内容を描画する（チケット非依存のため再送コストは小さい）。
   - `result_file` = `<projectRoot>/state/results/<page_id>.json`（ディスパッチ前に前回分を削除）。
5. **claude 起動**:
   - cwd = worktree パス
   - `claude -p <prompt> --output-format json` + （ネイティブ resume 時は `--resume <session_id>`） + （`config.systemPromptTemplate` 設定時は `--append-system-prompt <描画結果>`） + `config.agent.claude.args`（デフォルト `["--permission-mode", "bypassPermissions"]`）
   - `session_id` は `claude` の stream-json 出力から抽出し state に記録する（`opencode`/`grok`/`codex` も同様にそれぞれのネイティブ resume フラグ・`session_id` 抽出方式でセッション継続する。`takt` は現状 resume 未対応で毎回新規セッション）。
...
7. **成功時の Notion 更新**:
   ...
   - 最後にページを再取得して `last_edited_time` を記録（自分の更新分を取り込む。
     rework 判定の基準時刻）。
8. **失敗時**:
   - attempt < maxAttempts → retry_queued（バックオフ）。ページコメントに
     `⚠️ 失敗 (attempt N/M)、リトライ待ち: <短いエラー>` を投稿。
   - attempt >= maxAttempts → failed。レーンは動かさない。ページコメントに
     `❌ 自動実装に失敗しました (attempt N/M)` + エラー詳細（ログ末尾 ~1000字）を投稿。
     state に failed + そのページの last_edited_time を記録。
...
- **プロンプト**: `prompts/task.md` の `{{rework}}` に、前回 PR の URL・フィードバックコメント・
  ...
  通常実行では空文字。
- **セッション**: `human_rework` は前回 `session_id` があっても resume せず常に新規セッション
  （人間がチケット本文を編集した = 方向性が変わりうる差し戻しのため、前回セッションの前提を
  引きずらない）。`{{rework}}` を注入した `promptTemplate`（チケット全文）で dispatch する。
...
PR あり成功後、`state=done` + `prWatch` サブオブジェクトで PR を監視し、CI 失敗への自動対応を行う。
PR のマージ/クローズ検知は行わない（監視終了は Kanban 側が terminalLanes に到達した時点で
`terminalCleanup` が state を丸ごと削除する形に委ねる。マージ後の運用はチーム/ブランチ戦略に
委ね、本ツールは関与しない）。実装: `src/use-cases/orchestrator.ts` の `prReconcile` /
...
- 1 ページにつき `gh pr view <url> --json statusCheckRollup,headRefOid` を 1 コール。
- gh / ntn の失敗はページ単位で warn して continue（次回ポーリングで再試行）。

### 判定（decidePrWatchAction, 優先順）

1. **awaitingHuman**: advancePrWatch がポーリング対象から除外するため decidePrWatchAction には渡らない。
2. **CI 失敗**（phase 不問）: 失敗 check があり、
   - `headSha === reworkedSha`（この SHA は対応済み）→ 何もしない（再発火防止）。
   - `autoReworkCount >= config.autoReworkLimit`（デフォルト 3、トップレベル）→ **ci_limit**:
     `awaitingHuman=true` にして以後ポーリング対象外。🆘 失敗 check 一覧のコメントを
     **1 回だけ** 通知。人間の編集（human rework）でのみ解除される。
   - それ以外 → **ci_rework**: 失敗 check のログ（GitHub Actions は
     ...
     `autoReworkCount += 1`。
3. **CI 実行中**（pending あり）: 何もしない。
4. **全グリーン**: `phase:"ci"` なら **ci_green** → `phase:"review"` へ遷移、
...
- 順序は「スロット確認 → 再発火防止マーカー（reworkedSha）persist →
  dispatch」。スロット満杯/シャットダウン中ならマーカーを進めず次回へ持ち越し。
  マーカーを dispatch 起動前に persist するのは、クラッシュ時に「rework されない」側へ
  倒すため（二重 rework 防止を優先）。
- rework 成功時（onSuccess）に自分がレーンを動かした直後に結果整合性で古いレーンが返っても
  reconcile に殺されないよう、active entry は `dispatchedByUs=true` で登録。
- rework 成功時（onSuccess）に prWatch を再アーム: `phase:"ci"` に戻し、`autoReworkCount` は
  ci_failure 起因の rework のときのみ維持（他は 0 リセット）。reworkedSha は常に引き継ぐ
  （push なし success の同一 SHA 再発火を防ぐ）。awaitingHuman は引き継がない
  （rework 成功 = 人間介入の結果として自然解除）。
...
- **Notion 反映**: レーンは動かさない。`questionAskedAt = now` を**コメント投稿前に確定** →
  質問をページコメントに投稿（「返信すると自動で再開します」の案内付き）→
  ページ再取得で `lastEditedTime` を記録（自分の更新を「人間の本文編集」と誤検知しないため）。
...
- **再開**: attempt=1 の resume（`kind:"needs_info_answer"`）として dispatch。プロンプト先頭に
  ...
  `ci_failure` と同様、前回 `session_id` が記録されていればネイティブ resume +
  `resumePromptTemplate`（軽量プロンプト）を使う。
...
```jsonc
{
  ...
  "resumePromptTemplate": "prompts/resume.md",  // ci_failure/needs_info_answer でネイティブ resume する際の軽量プロンプト（チケット全文を含まない）
