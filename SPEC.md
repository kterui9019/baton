...
2. **リポジトリ解決**:
   - リポジトリ名 → `config.repoMapping[名前]` があればそれ、なければ名前そのまま → `config.repoRoot`（デフォルト `~/repos`）配下のディレクトリ。
   - ディレクトリが存在しない → `git clone <config.gitRemotePrefix><名前>.git <repoRoot>/<名前>` を試行（`config.autoClone: true` のとき。デフォルト true）。失敗したら試行失敗として扱う。
3. **Workspace 作成** (git worktree):
   ...
   - ブランチ名: `config.branchTemplate`（デフォルト `feature/notion-{id}/{slug}`）。`{id}` = page_id 先頭8文字（ハイフン除去後）、`{slug}` = タスクタイトルの sanitize（`[A-Za-z0-9._-]` 以外を `-` に、連続 `-` 圧縮、先頭末尾 trim、小文字化、最大40文字。空になったら `task`）。
...
```jsonc
{
  // 必須（validateConfig が起動時に検証。不足していれば全件表示して exit(1)）
  "repoRoot": "~/repos",
  "gitRemotePrefix": "git@github.com:your-org/",  // autoClone: true のとき必須
  ...
  // リポジトリ・ワークスペース
  "repoMapping": { "notion-repo-name": "local-dir-name" },
  "autoClone": true,
  "branchTemplate": "feature/notion-{id}/{slug}",
  "setupTimeoutMs": 600000,
  "repoSetup": {
    "your-repo": { "copy": [".env"], "commands": ["bun install"] }
  },
  ...
  src/
    ...
    interface-adapters/
      ...
      github/github-code-host-adapter.ts     # CodeHostPort の実装（gh ラッパー: PR スナップショット/
                                              # レビュー/失敗ログ取得）
      git/git-worktree-adapter.ts            # WorkspacePort の実装（リポジトリ解決, clone,
                                              # worktree 作成/削除, repoSetup）
